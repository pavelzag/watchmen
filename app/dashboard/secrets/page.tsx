...
import type { Secret, GcpSnapshot } from "@/lib/gcp/types";

type SortField = "name" | "createTime";
type SortDir = "asc" | "desc";

const PUBLIC_MEMBERS = new Set(["allUsers", "allAuthenticatedUsers"]);

function secretShortName(name: string) {
...
export default function SecretsPage() {
  ...
  useEffect(() => {
    fetch("/api/gcp/snapshot")
      .then((r) => r.json())
      .then((snap: GcpSnapshot) => setSecrets(snap.secrets ?? []))
  ...
  const memberCount = (s: Secret) =>
    new Set(s.iamPolicy.bindings.flatMap((b) => b.members)).size;

  const isPublic = (s: Secret) =>
    s.iamPolicy.bindings.some((b) => b.members.some((m) => PUBLIC_MEMBERS.has(m)));
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800/60">
                <SortHeader label="Secret" field="name" current={sortField} dir={sortDir} onSort={handleSort} />
                <th className="px-4 py-3 text-left text-xs uppercase tracking-wider text-slate-400">Project</th>
                <SortHeader label="Created" field="createTime" current={sortField} dir={sortDir} onSort={handleSort} />
            ...
            <tbody>
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      selected?.name === secret.name && "bg-sky-500/10"
                    )}
                  >
                    <td className="px-4 py-3 font-mono text-slate-200 text-xs">{secretShortName(secret.name)}</td>
                    <td className="px-4 py-3">
                      <span className="px-1.5 py-0.5 rounded text-xs bg-slate-700/60 text-slate-300">{secret.projectId}</span>
                    </td>
                    <td className="px-4 py-3 text-slate-400 text-xs">{fmt(secret.createTime)}</td>
                    <td className="px-4 py-3 text-xs text-slate-400">{memberCount(secret)}</td>
                    <td className="px-4 py-3">
                      {pub ? (
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500 text-sm">No secrets found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <DetailDrawer
        ...
        title={selected ? secretShortName(selected.name) : ""}
        subtitle={selected?.projectId}
      >
        {selected && (
          <>
            <DrawerSection label="Secret Details">
              <DrawerField label="Name" value={secretShortName(selected.name)} mono />
              <DrawerField label="Full Path" value={selected.name} mono />
              <DrawerField label="Project" value={selected.projectId} mono />
              <DrawerField label="Created" value={fmt(selected.createTime)} />
              <DrawerField label="Replication" value={selected.replicationPolicy} mono />
            </DrawerSection>
