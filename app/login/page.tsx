...
import { Github, KeyRound, ShieldAlert, Zap } from "lucide-react";
...
const HAS_GOOGLE_AUTH = hasUsableEnv(process.env.GOOGLE_CLIENT_ID, "your_google_client_id") &&
  hasUsableEnv(process.env.GOOGLE_CLIENT_SECRET, "your_google_client_secret");
const HAS_GITHUB_AUTH = hasUsableEnv(process.env.GITHUB_CLIENT_ID, "your_github_client_id") &&
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-sm px-6">
              ...
              {LOCAL_AUTH_ENABLED && (
                <form
                  ...
                  action={async (formData) => {
                    "use server";
                    await signIn("local", {
                      email: String(formData.get("email") || LOCAL_AUTH_EMAIL),
                      password: String(formData.get("password") || ""),
                      redirectTo: "/dashboard",
                    });
                  }}
                >
                  <input type="hidden" name="email" value={LOCAL_AUTH_EMAIL} />
                  {LOCAL_AUTH_PASSWORD && (
                    <input
                      name="password"
                      type="password"
                      ...
                      style={{ border: "1px solid #005c16", color: "#00ff41" }}
                    />
                  )}
                  <button
                    type="submit"
        ...
        <div className="mt-4 flex items-center justify-center gap-2 text-xs" style={{ color: "#003010" }}>
          ...
          <span>WATCHMEN v0.7.0 · CLOUD SECURITY EXPLORER</span>
        </div>
      </div>
      <Footer />
    </div>
