...
export async function executeTerraformPrFlow(
  ...
  if (!remediationPlan.fullyAddressed) {
    ...
    logTerraformWarn(scope, "partial_remediation_continues", {
      repoFullName,
  ...
  let prBody = `## Watchmen Security Remediation

This pull request was automatically generated by [Watchmen](https://github.com/pavelzag/watchmen) to address the following attack paths detected in your cloud infrastructure.
...
> **Note:** Review all changes carefully before merging. The AI-generated fixes apply the principle of least privilege but may need adjustment for your specific environment.`;
  ...
  if (remediationPlan.uncoveredTargets.length > 0) {
    const uncoveredDetails = remediationPlan.uncoveredTargets
      .map((target) => `- ${target.title}`)
      .join("\n");
    prBody += `\n\n### Uncovered Items\nThis PR includes the fixes Watchmen could generate automatically. ${remediationPlan.uncoveredTargets.length} selected item${remediationPlan.uncoveredTargets.length === 1 ? "" : "s"} were not fully covered and should be reviewed separately.\n${uncoveredDetails}`;
  }
